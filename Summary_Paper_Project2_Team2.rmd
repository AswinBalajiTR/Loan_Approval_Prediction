...
This dataset has some limitations, including an imbalance between numerical and categorical data, possible correlations between financial features. Dates are in the future, so they were not used in the model. Its a synthetic data created through algorithms or simulations rather than being collected from actual events or observations.
...
```
**Key Observations:**</br>
There are no redundant records in the dataset, meaning each observation is unique. This is crucial to ensure that the analysis is not skewed by redundant data, leading to accurate and reliable results.
...
**Key Observations:**</br>
The data shows a mix of distributions across variables—some, like Annual Income, are skewed due to the nature of the variable (e.g., income disparities), while others, like Credit Score, are more normally distributed. This variation reflects the inherent characteristics of each feature.
...
# Ensure 'RepaymentStatus' is defined
loan_data <- df %>%
  mutate(RepaymentStatus = ifelse(LoanApproved == 1, "Repaid", "Defaulted"))

# Repayment Behavior by Loan Purpose (Stacked Bar Chart)
loan_data %>%
  group_by(LoanPurpose, RepaymentStatus) %>%
  summarise(Count = n()) %>%
  mutate(Proportion = Count / sum(Count)) %>%
  ggplot(aes(x = LoanPurpose, y = Proportion, fill = RepaymentStatus)) +
...
# Risk Score vs Repayment Status (Boxplot)
ggplot(loan_data, aes(x = RepaymentStatus, y = RiskScore, fill = RepaymentStatus)) +
  geom_boxplot() +
  labs(title = "Risk Score vs Repayment Status",
       x = "Repayment Status",
       y = "Risk Score") +
  theme_minimal() +
  scale_fill_manual(values = c("Defaulted" = "skyblue", "Repaid" = "salmon"))
...
predictors <- df[, !(names(df) %in% "LoanApproved")]
response <- as.factor(loan_data$LoanApproved)  # Convert target to a factor

# Fit a GLM (logistic regression) model
glm_model <- glm(response ~ ., data = predictors, family = binomial)

# Extract feature importance (absolute coefficients)
importance <- abs(coef(glm_model)[-1])  # Exclude the intercept
features <- names(coef(glm_model))[-1]
...
ggplot(feature_importance, aes(x = reorder(Feature, Importance), y = Importance)) +
  ...
  coord_flip() +
  labs(title = "Feature Importance for Loan Approval Prediction (GLM)",
       x = "Features",
...
The strongest predictors of loan approval or rejection include factors such as credit score, which indicates financial responsibility, and debt-to-income ratio (DTI), reflecting the borrower’s ability to manage debt. Income level and employment history also play a key role in demonstrating financial stability. Additionally, the loan amount requested and existing debt load impact the decision, with larger loans and high existing debt reducing approval chances. These factors help lenders assess the borrower’s repayment ability and overall risk.</br>
...
**Key Observations:**</br>
The logistic regression model achieves exceptional performance, with an AUC of 1.0, demonstrating perfect discrimination between approved and rejected loans. It also achieves a high accuracy of 99.8% on both the training and testing sets, with minimal misclassification errors (False Positives: 5, False Negatives: 3). Significant predictors include RiskScore, AnnualIncome, and DebtToIncomeRatio, as indicated by their coefficients. The model's high sensitivity (99.8%) and specificity (99.7%) make it highly reliable for predicting loan approvals while minimizing both false approvals and rejections. While it lacks intuitive interpretability, the logistic regression model is ideal for robust and precise predictions.
...
The decision tree model provides an interpretable framework for loan approval prediction, achieving an AUC of ~0.98 and high accuracy (98.8% testing). It identifies key decision points such as RiskScore, AnnualIncome, and DebtToIncomeRatio, which offer clear, stakeholder-friendly rules. However, it shows slightly higher misclassification errors (False Positives: 25, False Negatives: 24) compared to the logistic regression model, which could lead to a higher chance of incorrect predictions. Despite this, the decision tree's simplicity and visual decision-making process make it suitable for applications where interpretability is crucial.

# **Overall Analysis of Model Strengths and Limitations **</br>
# **Conclusion**</br>

While age does not significantly impact loan approval or risk, financial factors such as total assets and income play a crucial role, with low-risk applicants typically having higher assets and more varied income. Employment status is a key determinant, as employed individuals tend to have lower risk scores and higher approval rates, while unemployed or self-employed applicants face higher risk and reduced approval chances. Marital status does not appear to have a significant effect on loan approval or risk.

Both the decision tree and logistic regression models perform exceptionally well, with the logistic regression model slightly outperforming in terms of accuracy (99.8% vs. 98.8%), sensitivity, specificity, and AUC (1.0 vs. ~0.98 for the decision tree). The logistic regression model demonstrates fewer false positives (FP: 5) and false negatives (FN: 3) compared to the decision tree (FP: 25, FN: 24) on the testing set, reflecting its better balance and precision. While the decision tree offers interpretability through a visual flow of decision-making, the logistic regression model is more robust and generalizes better, making it the preferred choice for maximizing predictive accuracy and minimizing classification errors.

# **References **</br>

1. Johnson, T. R., Smith, P. M., et al. (2023). Predictive Modeling for Loan Approval: Balancing Accuracy and Fairness in Machine Learning Approaches.
Journal of Financial Technology.  [Link](https://www.researchgate.net/publication/385265203_Study_on_Predictive_Modeling_for_Loan_Approval)

2. Chen, L., Davis, H. A., et al. (2023). Leveraging Socioeconomic Data in Loan Risk Assessment: Insights from Explainable AI.
International Journal of Data Science in Finance.  [Link](https://www.researchgate.net/publication/382593102_Explainable_artificial_intelligence_XAI_in_finance_a_systematic_literature_review)
